...
export default class BattlePlayerPanel extends Panel {
    ...
    constructor(x, y, width, height, pokemon, options = {}) {
        ...
        // Create health bar with text indicator
        this.healthBar = new ProgressBar(
            this.position.x + 65,  
            this.position.y + this.dimensions.y - 42,  
            130,  
            8,    
            this.displayHealth,
            pokemon.health
        ...
        // Create experience bar BELOW the panel
        this.experienceBar = new ProgressBar(
            this.position.x + 65, 
            this.position.y + this.dimensions.y + 5,  
            this.dimensions.x - 75,
            8,   
            pokemon.currentExperience,
            pokemon.targetExperience,
            { singleColor: 'rgb(64, 160, 255)' } 
        );
        
        this.isGainingExperience = false;
    ...
    /**
     * Animate the experience bar and play sound
     * @param {number} amount - Amount of experience to gain
     * @param {function} callback - Called when experience gain is complete
     */
    gainExperience(amount, callback = () => {}) {
        if (this.isGainingExperience) return;
        
        this.isGainingExperience = true;
        const startExp = this.pokemon.currentExperience;
        const targetExp = startExp + amount;
        const expDiff = amount;
        ...
        const updateTask = timer.addTask(() => {
            elapsed += interval;
            const progress = Math.min(elapsed / duration, 1); 
            const currentExp = startExp + (expDiff * progress);
            this.experienceBar.setValue(currentExp);
            
            if (progress >= 1) {
                this.pokemon.currentExperience = targetExp;
                this.experienceBar.setValue(targetExp);
                this.isGainingExperience = false;
    ...
    renderStatistics() {
        ...
        context.fillText(
            'HP:',
            this.position.x + 30,
            this.position.y + this.dimensions.y - 39 
        );
        ...
        context.textAlign = 'right';
        context.fillText(
            `${Math.floor(this.displayHealth)}/${this.pokemon.health}`,
            this.position.x + this.dimensions.x - 10,
            this.position.y + this.dimensions.y - 30  
        );
